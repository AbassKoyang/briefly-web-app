...
const NavBar = () => {
  ...
  return (
    <header className='w-full bg-white py-3 md:py-2 px-4 md:px-8 flex items-center justify-between'>
        <div className="flex items-center gap-2">
        <button onClick={() => setIsSidebarOpen(true)} className="block lg:hidden p-2 md:p-2.5 bg-white border border-gray-300 rounded-md relative cursor-pointer">
            <Menu className='size-[18px] lg:size-[20px] text-gray-800' />
        </button>
            <div className="border-1 border-gray-300 rounded-lg overflow-hidden w-[180px] md:w-[300px] group focus-within:border-gray-600 flex items-center justify-between px-4 transition-all duration-200 ease-in-out">
                <Search className='group-focus-within:text-gray-800 text-gray-500 size-[16px] transition-all duration-200 ease-in-out' />
                <input type="text"                 value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)} className="w-[90%] h-[35px] md:h-[40px] px-2 text-sm text-gray-800 font-nunito-sans placeholder:text-gray-500 border-0 stroke-0 outline-0 transition-all duration-200 ease-in-out" placeholder='Search by title...'/>
            </div>
        </div>
        ...
        <div className="">
            {user ? (
                    <div className='flex items-center gap-2'>
                        <button onClick={() => setIsFormOpen(true)} className='px-2 md:px-4 py-2 rounded-md bg-dark-blue text-white font-medium font-raleway text-sm flex items-center gap-1.5 cursor-pointer'>
